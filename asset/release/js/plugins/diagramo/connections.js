...
function Connector(startPoint,endPoint,type, id){
    ...
    this.style.lineWidth = 3.005;   // fixes connector's display in Chrome for now // modify by zengwei 1.005
 ...
 *@author Alex Gheorghiu <alex@scriptoid.com>
 */
var connectionParentRelNum = {};
function ConnectionPoint(parentId, point, id, type, portNum) {
    /**Connection point id*/
    this.id = id;
    ...
    if (portNum == undefined || portNum == -1) {
        if (connectionParentRelNum[parentId] == null) {
            connectionParentRelNum[parentId] = 1;
        } else {
            ++connectionParentRelNum[parentId];
        }
        this.portNum = connectionParentRelNum[parentId];
    }else{
...
ConnectionPoint.load = function(o){
    var newConnectionPoint = new ConnectionPoint(o.parentId, new Point(0, 0), o.id, ConnectionPoint.TYPE_FIGURE, o.portNum); //fake constructor
 ...
 **/
function Glue(cp1Id, cp2Id, automatic, isStart) {
