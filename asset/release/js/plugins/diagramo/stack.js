...
//            this.reindex();
    ...
    figureRemoveById :function(figId){
        ...
        for(var i=0; i<this.figures.length; i++ ){
            if(this.figures[i].id == figId){
                ...
                if(figure.unDoubleClickFunc != undefined){
                    figure.unDoubleClickFunc();
                }
                break;
            }
        }
    ...
    figureGetRightByGroupId:function(groupId){
        var groupFigures = [];
        for(var i=0; i<this.figures.length; i++){
            if(this.figures[i].groupId == groupId){
                groupFigures.push(this.figures[i]);
        ...
        var retFigure = null
        for(var i=0; i<groupFigures.length; i++){
            var figure = groupFigures[i];
            var fpoint = figure.getPoints()
            var fristPoint = fpoint[0]

            if(retFigure == null || retFigure.getPoints()[0].x > fristPoint.x){
                retFigure = figure
            }
    ...
    figureGetLeftByGroupId:function(groupId){
        var groupFigures = [];
        for(var i=0; i<this.figures.length; i++){
            if(this.figures[i].groupId == groupId){
                groupFigures.push(this.figures[i]);
