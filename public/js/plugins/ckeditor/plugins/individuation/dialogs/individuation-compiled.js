...
(function () {
    ...
    CKEDITOR.dialog.add('individuationDialog', function (editor) {
        ...
        //获取groupKey
        var getGroupKey = function getGroupKey() {
            return $("#vipGroup").val();
        };

        //获取个性化数据
        var createIndividuationData = function createIndividuationData() {
            var nonJumpHrefData;
        ...
        var judgePort = function judgePort() {
            var parentNode;
            if (_settingSpan) {
        ...
        //创建个性化内容
        var createIndiv = function createIndiv() {
            var inCon = document.createElement("div");
            inCon.className = "indivCon";
            var txt = $("<div class='indivTxt'></div>").append("<label style='display: inline-block;width: 50px;text-align: center;'>" + TxtContent.txt + "</label>" + "<input type='text' class='txtImg' style='margin-left: 10px;width: 75%;'>" + "<div class='extendPersonality' style='margin-top: 10px;'><label style='display: inline-block;width: 50px;text-align: center;'>" + TxtContent.ckeitor.skipHref + "</label>" + "<input type='text' class='jumpHref' style='margin-left: 10px;width: 75%;'></div>");
            var link = $("<div class='indivUrl'></div>").append("<div class='linkBox'>" + "<label style='width: 80px;text-align: right;display:inline-block;'>" + TxtContent.ckeitor.href + "：</label>" + "<input type='text' class='txtImg' style='width: 71%;'>" + "</div>" + "<label style='width: 80px;text-align: right;display:inline-block;position: relative;top: -33px;'>" + TxtContent.ckeitor.img + "：</label><div class='imgBox'>" + "<img class='previewImg'/>" + "</div>" + "<div class='extendPersonality' style='margin-top: 10px;'><label style='width: 80px;text-align: right;display:inline-block;'>" + TxtContent.ckeitor.skipHref + "：</label><input type='text' class='jumpHref' style='width: 71%'></div>");
        ...
        var createFilter = function createFilter() {
            var $wrap;
            var titConditon = "<p style='border-top: none;position: relative;'>" + TxtContent.ckeitor.ruleCondition + "：" + "<a href='#' class='addConditon'>" + "<span class='glyphicon glyphicon-plus'>" + TxtContent.ckeitor.add + "</span>" + "</a>" + "<span class='saveCondition'>" + TxtContent.ckeitor.save + "</span>" + "</p>";
            var condition = createConditon();
            $wrap = $("<div class='conditionWrap'></div>");
            var conditionBox = $wrap.append(condition);
            var titVip = "<p>" + TxtContent.ckeitor.des + "：<span class='matchCount' style='margin-right: 5px'>" + "<span class='matchVipCount'>0</span></span>" + "</p>";
        ...
        var createEle = function createEle() {
            ...
            //分组盒子
            var groupCon = "<lable>" + TxtContent.ckeitor.group + "：</lable>" + "<select name='smartgroupKey' id='vipGroup' style='display: block;padding: 6px 12px;" + "width: 100%;margin-top: 5px;height: 30px;'>" + "<option value='' count=''>所有会员</option>" + "</select>";
            var groupBox = $("<div class='groupKey'></div><hr class='one'/>").append(groupCon);

            //不符合条件的vip
            var filterVip = "<p id='spanCon'>" + TxtContent.ckeitor.defCon + "：<input id='spanText'></p>" + "<p id='filterVip'>" + TxtContent.ckeitor.nonRuleCount + "：" + "<span class='filterVipCount' style='margin-right: 5px;'></span>" + "<a id='extendHref' href='javascript:void(0);' style='color:#23C6C8;margin-left: 20px;display: none;'>" + TxtContent.ckeitor.extendHref + "</a>" + "</p>" + '<div class="extendPersonality" style="margin-top: 10px;"><label style="margin-left: 20px;font-size: 14px;">' + TxtContent.ckeitor.defSkipHref + '：</label>' + '<input type="text" class="nonJumpHref" style="width: 80%"></div>' + "<a id='addsettingJson'>" + TxtContent.ckeitor.addRule + "</a>" + '<hr class="one">';
        ...
        //删除父元素
        var removeParentEle = function removeParentEle(obj) {
            obj.parent().remove();
        };
        ...
        var createRule = function createRule() {
            ...
            settingJsonWrap.find(".saveCondition").off().on("click", function () {
                ...
                //获取nodeValue
                var getNodeValue = function getNodeValue() {
                    var nodeValue = {};
                    if (element.getName() == "img") {
        ...
        //加载个性化
        var loadIndividuation = function loadIndividuation(obj) {
            //加载个性化数据
            var indivData = obj;
            if (indivData) {
                var nonNodeVal;
                ...
                if (isImg) {
                    ...
                    personalityContent.after('<button id="checkImg" data-container="body" ' + 'data-toggle="popover" data-placement="bottom" ' + 'data-content="<img src=' + nonNodeVal + '>" style="margin-left:5px;' + 'color: #23C6C8;">"+TxtContent.check+"</button>');
                }
        ...
        //更新数据
        var updateSetting = function updateSetting(individuationData, settingSpan) {
        ...
        //设置spanId
        var setSpanId = function setSpanId(msg, settingSpan) {
        ...
        //添加
        var addSettings = function addSettings(individuationData, settingSpan) {
        ...
        return {
            ...
            buttons: [{
                ...
                onClick: function onClick() {
                    new FormModal({
                        ...
                        "submitBindFunc": function submitBindFunc() {
                ...
                onClick: function onClick() {
                    ...
                    //只有所有节点全被保存才执行刷新
                    var refreshNode = function refreshNode() {
            ...
            onShow: function onShow() {
                ...
                if (!$(element.getParent().$).data("nav") && !$(element.$).data("nav") && !element.hasClass("editorImg")) {
                    ...
                    if (selTxt && element) {
                        if (element.getName() == 'td' || element.getName() == 'p' || element.getName() == 'span' || element.getName() == 'strong' || element.getName() == 'h3' || element.getName() == 'li' || element.getName() == 'div') {
                    ...
                    if (element) {
                        if (element.getName() == 'img') {
                            ...
                            $spanVal.val(srcVal).after('<button id="checkImg" data-container="body" ' + 'data-toggle="popover" data-placement="bottom" ' + 'data-content="<img src=' + srcVal + '>" style="margin-left:5px;' + 'color: #23C6C8;">' + TxtContent.check + '</button>');
                            $("#addsettingJson").after('<div style="text-align: center;margin-top: 8px;color: #ed5565;">' + TxtContent.ckeitor.condition + '</div>');
                        } else {
                            if (!selTxt) {
                ...
                //region 加载会员分组
                //添加option
                var addMemberOption = function addMemberOption(obj) {
                    var member = $("#vipGroup");
                    //添加options
...
//# sourceMappingURL=individuation-compiled.js.map