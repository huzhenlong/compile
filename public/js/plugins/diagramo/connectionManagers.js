...
//            }
     ...
     * add by zengwei
     */
    getOrderPointsOfConnection: function(figuerId){
        var figuerObj = STACK.figureGetById(figuerId)
        var connectionPoints = this.connectionPointGetAllByParent(figuerId)
        var i = 1;
        if(figuerObj.figureType == 1 ){
            i = 0;
        }
        for( ; i < connectionPoints.length ; i++) {
            var curPoint = connectionPoints[i];
            var lastPoint = curPoint.point;
            if (lastPoint.hasLine == undefined || lastPoint.hasLine == 0) {
                lastPoint.hasLine = 1
                var cpLastPoint = lastPoint.clone()
                return [cpLastPoint, cpLastPoint, curPoint.id, curPoint.id];
            }
        }
    },
    /**
...
//        Log.level = Log.LOG_LEVEL_DEBUG; 
    ...
    //add by zengwei
    connectionPointGetIdPortNumByXY: function (x, y, type) {
        var id = -1;
        var portNum = -1;
        ...
        for (var i = 0; i < this.connectionPoints.length; i++) {
            if (this.connectionPoints[i].contains(x, y) && this.connectionPoints[i].type == type) {
                id = this.connectionPoints[i].id;
                portNum = this.connectionPoints[i].portNum;
                break;
            }
        }
    ...
    //add by zengwei
    connectionPointIsOver: function (x, y) {
        var found = false;
        for (var i = 0; i < this.connectionPoints.length; i++) {
            if (this.connectionPoints[i].contains(x, y) && this.connectionPoints[i].type == type) {
                found = true;
    ...
    connectionPointCreate:function(parentId, point, type){
        var idPortNum = CONNECTOR_MANAGER.connectionPointGetIdPortNumByXY(point.x, point.y, ConnectionPoint.TYPE_FIGURE);
        var conPoint = new ConnectionPoint(parentId, point.clone(), this.connectionPointCurrentId++, type, idPortNum.portNum);
        this.connectionPoints.push(conPoint);
        
        return conPoint;
    },

    connectionPointCopyPortNum: function (x, y) {
        var idPortNum = CONNECTOR_MANAGER.connectionPointGetIdPortNumByXY(x, y, ConnectionPoint.TYPE_FIGURE);
        return idPortNum;
    },
    ...
    connectionPointRemoveAllByParent:function(parentId){
        delete connectionParentRelNum[parentId];
        for(var i=0; i<this.connectionPoints.length; i++){
    ...
    connectionPointRemoveCache: function (parentId) {
        delete connectionParentRelNum[parentId];
    },
     ...
     * @returns {Array}
     */
    setHasLineUseConnectionPoint:function(pointPosition){
        var collectedPoints = [];

        for(var connectionPoint in this.connectionPoints){
            var point = this.connectionPoints[connectionPoint].point
            if(point.x == pointPosition.x && point.y == pointPosition.y){
                point.hasLine = 0
        ...
        for(var i = 0 ; i < fCps.length; i++){
            ...
            for(var j = 0; j < gluesLength; j++){
                ...
                var candidate = CONNECTOR_MANAGER.getClosestPointsOfConnection(
                    0, //start automatic modify by zengwei start point no move
                    0,//automaticEnd, //end automatic
...
/*
     ...
     *@author Alex Gheorghiu <alex@scriptoid.com>
     **/
    glueCreate: function (firstId, secondId, automatic, isStart) {
        var glue = new Glue(firstId, secondId, automatic, isStart);
