...
var outFigureCount = null;

var outFigureType = null;
 ...
 **/
function createFigure(fFunction, thumbURL,outCount,figureType){
    ...
    outFigureCount = outCount;

    outFigureType = figureType;
...
function delConnector(){
    ...
    if(selectedConnectorId != -1){
        ...
        cmdDelCon.execute();
        //modiby by zengwei remove hasLine
        var points = cmdDelCon.connectionpoints;
        for(var i = 0 ; i < points.length ; i ++){
            var p = points[i];
            CONNECTOR_MANAGER.setHasLineUseConnectionPoint(p.point);
        }
...
var isCtrlMouseFirstDown = 0
function ctrlLineCreate(x,y,ev,id){
    selectedFigureId = id;
    state = STATE_CONNECTOR_PICK_FIRST;
    action('connector-organic')
    if(isCtrlMouseFirstDown == 1){
        var ret = connectorPickFirst(x,y,ev);
        if(ret == -1){
            escFunction()
            mousePressed = false;
            CNTRL_PRESSED = false;//去除ctrl弹不起来
            alert("没有可用节点")
            return -1;
        }
        isCtrlMouseFirstDown = 0
    }else{
        connectorPickFirst(x,y,ev);
        isCtrlMouseFirstDown = 1
    }
    return 1;
}
/**
        ...
        case STATE_NONE:
            ...
            switch(selectedObject.type) {
                ...
                case 'Group':
                    ...
                    break;
                case 'Figure':
                    if(CNTRL_PRESSED){// no finish by zengwei
                        var ret = ctrlLineCreate(x,y,ev,selectedObject.id)
                        if(ret == -1){
                            break
                        }
                    }else{
                ...
                case 'Container':
                    ...
                    redraw = true;
                    break;
                case 'ConPoint'://add by zengwei
                    //var selectedConPointId = selectedObject.id;
                    state = STATE_CONNECTOR_PICK_FIRST;
                    action('connector-organic');
                    connectorPickFirst(x,y,ev);
                    Log.info('onMouseDown() + STATE_NONE  - change to STATE_CONTAINER_SELECTED');
...
function onMouseUp(ev){
    ...
    switch(state){
        ...
        */
        ...
        /*case STATE_CONNECTOR_PICK_SECOND:  add by zengwei
...
*/
        ...
        case STATE_CONNECTOR_MOVE_POINT:
            ...
            //reset all {ConnectionPoint}s' color
            CONNECTOR_MANAGER.connectionPointsResetColor();
            CONNECTOR_MANAGER.connectionPointCopyPortNum(x, y);
    ...
    if(cId != -1){
        ...
        if (cId != -1) {
            ...
            if (fId != -1) {
                ...
                //zengwei add double click
                if(figure != undefined && figure.doubleClickFunc != undefined){
                    figure.doubleClickFunc(figure)
                }
            } else {
...
function connectorPickFirst(x, y, ev){
    ...
    if(fCpOverId == -1 && fOverId !== -1){
        candidate = CONNECTOR_MANAGER.getOrderPointsOfConnection(fOverId);

        if(candidate == undefined || candidate == null){
            Log.info("no have point selected ");
            return -1;
        }
    }
    ...
    if(fCpOverId != -1){ //Are we over a ConnectionPoint from a Figure?
        ...
        var g = CONNECTOR_MANAGER.glueCreate(fCp.id, conCps[0].id, false, true);
        ...
        modify by zengwei add getOrderPointsOfConnection method
        */
        ...
        var g = CONNECTOR_MANAGER.glueCreate(candidate[2], conCps[0].id, true, true);
        Log.info("First glue created : " + g);
    }
...
function connectorPickSecond(x, y, ev){
    ...
    //play with algorithm
    {
        ...
        var automaticStart = 0;//rStartGlues.length > 0 && rStartGlues[0].automatic;//add by zengwei start point
    ...
    if(fCpOverId != -1){ //Are we over a ConnectionPoint from a Figure?
        CONNECTOR_MANAGER.glueCreate(fCpOverId, CONNECTOR_MANAGER.connectionPointGetSecondForConnector(selectedConnectorId).id, false, false);
    } else if(fOverId != -1){ //Are we, at least, over a Figure?
        CONNECTOR_MANAGER.glueCreate(candidate[3]/*end Figure's ConnectionPoint Id*/, CONNECTOR_MANAGER.connectionPointGetSecondForConnector(selectedConnectorId).id, true, false);
...
//                alert(svgDiagram);
    ...
    if(workFlowId =="undefined"){
        ...
        currentDiagramId = workFlowId;
    }


    //see: http://api.jquery.com/jQuery.post/
    var ret = WORKFLOW.updateDiagram(serializedDiagram, brand, group, currentDiagramId, workFlowName);
...
function load(diagramId){
    ...
    var ret = WORKFLOW.getByIdWorkFlow(diagramId);
    if (ret.content != null) {
        var obj = eval('(' + ret.content + ')');
...
function addListeners(){
    ...
    if(false){
        ...
        var ontouchstart="touchStart(event);"
        var ontouchmove="touchMove(event);"
        var ontouchend="touchEnd(event);"
        var ontouchcancel="touchCancel(event);"
    }

}

/**Minimap section*/
var minimap = null; //stores a refence to minimap object (see minimap.js)
...
function init(diagramId) {
   ...
   var minimapVar = document.getElementById("minimap");
   if(minimapVar != null){
       minimap = new Minimap(canvas, minimapVar, 115);
       minimap.updateMinimap();
   }
   ...
   if(canvasProps == null){//only create a new one if we have not already loaded one
       canvasProps = new CanvasProps(ctx.width, CanvasProps.DEFAULT_HEIGHT, CanvasProps.DEFAULT_FILL_COLOR);
   }
   ...
   setUpEditPanel(canvasProps);
    ...
    $('#key').val(diagramId);
    if(typeof(diagramId) != "undefined"  && diagramId != null &&diagramId!=''){
        if (!load(diagramId)) {
            var fgs = figureSets["base"].figures;
            //add
            var cmdCreateFig = FigureCreateCommandBaseFigure(fgs[0], 100, 300);
            cmdCreateFig.execute();


            var cmdCreateFig = FigureCreateCommandBaseFigure(fgs[1], ctx.width - 200, 300);
            cmdCreateFig.execute();
        }
    }
...
//            break;
        ...
        case 'moveback':
            ...
            break;
        case 'moveright'://add by zengwei
            if(selectedGroupId != -1){
                var group = STACK.groupGetById(selectedGroupId);
                if(!group.permanent) { //group only temporary groups
                    var firstFigure = STACK.figureGetLeftByGroupId(selectedGroupId);
                    var fpoints = firstFigure.getPoints();
                    var fristPoint = fpoints[0]
                    var targetX = fristPoint.x

                    var figures = STACK.figureGetByGroupId(selectedGroupId)
                    for (var i = 0; i < figures.length; i++) {
                        var tmpFiure = figures[i]
                        var tmpFpoint = tmpFiure.getPoints()
                        var tmpFristPoint = tmpFpoint[0]
                        var translateMatrix = Matrix.leftMatrix(targetX - tmpFristPoint.x);
                        var cmdGroup = new FigureTranslateCommand(tmpFiure.id, translateMatrix);
                        cmdGroup.execute();
                        History.addUndo(cmdGroup);
                    }
                    Log.info("main.js->action()->Group. New group made permanent. Group id = " + selectedGroupId);
            ...
            redraw = true;
            break;
        case 'moveleft'://add by zengwei
            if(selectedGroupId != -1){
                var group = STACK.groupGetById(selectedGroupId);
                if(!group.permanent) { //group only temporary groups
                    var firstFigure = STACK.figureGetRightByGroupId(selectedGroupId);
                    var fpoints = firstFigure.getPoints();
                    var fristPoint = fpoints[0]
                    var targetX = fristPoint.x
                ...
                switch(state){                                
                    case STATE_FIGURE_CREATE:
                        ...
                        if(window.createFigureFunction){
                            ...
                            var cmdCreateFig = new FigureCreateCommand(window.createFigureFunction, x, y,selectedFigureThumb,outFigureCount,outFigureType);
                            cmdCreateFig.execute();
                            History.addUndo(cmdCreateFig);
